...
<head>
<meta charset="UTF-8">
<link href="/board_JPA/static/css/propertyCloud.css" rel="stylesheet">
...
<body>
	...
	<script>
		...
		function sendData(host_name, scan_type, scan_key, access_key_id, region, scan_interval, namespaces, submitType){
			...
			if(submitType==="new"){
				url = url +"/board_JPA/process?type=new";
			}else if(submitType==="update"){
				url = url +"/board_JPA/process?type=update";
			}
			$.ajax({
				url: url,
				type:"POST",
				...
				success : function(response){
					if(response=="success"){
						opener.location.replace( $(location).attr("origin")+"/board_JPA/"+"?lang=${lang}");
						window.close();
					}else{
						if(submitType==="new"){
