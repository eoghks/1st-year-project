...
<head>
<meta charset="UTF-8">
<link href="/test/static/css/propertyCloud.css" rel="stylesheet">
...
<body>
	...
	<script>
		...
		$(document).ready(function(){
			if(${property}.data.host_name != null){
		...
		function sendData(host_name, scan_type, scan_key, access_key_id, region, scan_interval, namespaces, submitType){
			...
			$.ajax({
				...
				success : function(response){
					if(response=="success"){
						opener.location.replace( $(location).attr("origin")+"/test/"+"?lang=${lang}");
						window.close();
					}else{
